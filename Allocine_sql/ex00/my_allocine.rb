...
"SELECT rev_name FROM reviews, movies_ratings_reviews WHERE movies_ratings_reviews.rev_id = reviews.id AND movies_ratings_reviews.rev_stars >= 7 AND reviews.rev_name IS NOT NULL ORDER BY rev_name"
...
"SELECT actors.id, actors.act_fname, actors.act_lname, actors.act_gender FROM actors JOIN movies_actors ON actors.id = movies_actors.act_id JOIN movies ON movies_actors.mov_id = movies.id WHERE movies.mov_title = 'Annie Hall'"
...
"SELECT actors.act_fname, actors.act_lname, movies_actors.role FROM actors 
...
JOIN actors ON movies_actors.act_id = actors.id
